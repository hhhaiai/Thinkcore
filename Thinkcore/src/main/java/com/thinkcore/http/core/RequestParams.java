 ...
 * requests made from an {@link TAsyncHttpClient} instance.
 * <p>
 * &nbsp;
 * </p>
 * For example:
...
public class RequestParams {
	...
	public void setContentEncoding(final String encoding) {
		if (encoding != null)
			...
			TLog.d(LOG_TAG, "setContentEncoding called with null attribute");
	}
	...
	public void put(String key, InputStream stream, String name,
			String contentType) {
		put(key, stream, name, contentType, mAutoCloseInputStreams);
	}
	...
	public void put(String key, InputStream stream, String name,
		...
		if (key != null && stream != null) {
			mStreamParams.put(key, StreamWrapper.newInstance(stream, name,
	...
	public String toString() {
		...
		for (ConcurrentHashMap.Entry<String, String> entry : mUrlParams
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
		...
		for (ConcurrentHashMap.Entry<String, StreamWrapper> entry : mStreamParams
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
		...
		for (ConcurrentHashMap.Entry<String, FileWrapper> entry : mFileParams
				.entrySet()) {
			if (result.length() > 0)
				result.append("&");
	...
	public HttpEntity getEntity(ResponseHandlerInterface progressHandler)
		...
		} else if (mStreamParams.isEmpty() && mFileParams.isEmpty()) {
	...
	private HttpEntity createJsonStreamerEntity(
		...
		JsonStreamerEntity entity = new JsonStreamerEntity(progressHandler,
				!mFileParams.isEmpty() || !mStreamParams.isEmpty());
		...
		for (ConcurrentHashMap.Entry<String, Object> entry : mUrlParamsWithObjects
				.entrySet()) {
			entity.addPart(entry.getKey(), entry.getValue());
		}
	...
	private HttpEntity createMultipartEntity(
		...
		SimpleMultipartEntity entity = new SimpleMultipartEntity(
				progressHandler);
		entity.setIsRepeatable(mIsRepeatable);
	...
	protected List<BasicNameValuePair> getParamsList() {
		...
		lparams.addAll(getParamsList(null, mUrlParamsWithObjects));
