...
public abstract class AsyncHttpResponseHandler implements
	...
	private String mResponseCharset = DEFAULT_CHARSET;
	...
	public String getCharset() {
		return this.mResponseCharset == null ? DEFAULT_CHARSET
	 ...
	 */
	public void onProgress(int bytesWritten, int totalSize) {
		TLog.v(LOG_TAG, String.format("Progress %d from %d   completed:%2.0f",
				bytesWritten, totalSize,
				(totalSize > 0) ? ((bytesWritten * 1.0 / totalSize) * 100)
						: (-1 * 1.0)));
	}
	...
	public void onRetry(int retryNo) {
		TLog.d(LOG_TAG, String.format("Request retry no. %d", retryNo));
	}

	public void onCancel() {
		TLog.d(LOG_TAG, "Request got cancelled");
	}
	...
	protected void handleMessage(Message message) {
		...
		case SUCCESS_MESSAGE:
			...
			if (response != null && response.length >= 3) {
				...
				TLog.e(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
			}
			break;
		case FAILURE_MESSAGE:
			...
			if (response != null && response.length >= 4) {
				...
				TLog.e(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
			}
			break;
		case START_MESSAGE:
			onStart();
		...
		case PROGRESS_MESSAGE:
			...
			if (response != null && response.length >= 2) {
				...
				TLog.e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
			}
			break;
		case RETRY_MESSAGE:
			response = (Object[]) message.obj;
			if (response != null && response.length == 1)
				...
				TLog.e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
			break;
		case CANCEL_MESSAGE:
			onCancel();
			break;
	...
	protected Message obtainMessage(int responseMessageId,
		...
		if (mHandler == null) {
			...
			msg = Message.obtain(mHandler, responseMessageId,
					responseMessageData);
		}
		return msg;
	}
