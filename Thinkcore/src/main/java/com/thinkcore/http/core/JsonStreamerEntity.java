...
import com.thinkcore.utils.encryption.TBase64;
import com.thinkcore.utils.encryption.TBase64OutputStream;
...
class JsonStreamerEntity implements HttpEntity {
	...
	public void writeTo(final OutputStream out) throws IOException {
		...
		OutputStream os = null != contentEncoding ? new GZIPOutputStream(out,
				BUFFER_SIZE) : out;
		...
		TLog.i(TAG, "Uploaded JSON in " + Math.floor(elapsedTime / 1000)
				+ " seconds");

		// Flush the contents up the stream.
		os.flush();
		TAsyncHttpClient.silentCloseOutputStream(os);
	}

	private void writeToFromStream(OutputStream os,
			RequestParams.StreamWrapper entry) throws IOException {
		...
		TBase64OutputStream bos = new TBase64OutputStream(os, TBase64.NO_CLOSE
				| TBase64.NO_WRAP);

		// Read from input stream until no more data's left to read.
		while ((bytesRead = entry.inputStream.read(mBuffer)) != -1) {
			bos.write(mBuffer, 0, bytesRead);
		}

		// Close the Base64 output stream.
		TAsyncHttpClient.silentCloseOutputStream(bos);
		...
		if (entry.autoClose) {
			...
			TAsyncHttpClient.silentCloseInputStream(entry.inputStream);
		}
	}

	private void writeToFromFile(OutputStream os,
			RequestParams.FileWrapper wrapper) throws IOException {
		...
		while ((bytesRead = in.read(mBuffer)) != -1) {
			bos.write(mBuffer, 0, bytesRead);
		...
		// Safely close the input stream.
		TAsyncHttpClient.silentCloseInputStream(in);
	}
