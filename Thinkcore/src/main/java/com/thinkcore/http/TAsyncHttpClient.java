...
import com.thinkcore.http.core.AsyncHttpRequest;
import com.thinkcore.http.core.MyRedirectHandler;
import com.thinkcore.http.core.MySSLSocketFactory;
import com.thinkcore.http.core.PersistentCookieStore;
import com.thinkcore.http.core.PreemtiveAuthorizationHttpRequestInterceptor;
import com.thinkcore.http.core.RangeFileAsyncHttpResponseHandler;
import com.thinkcore.http.core.RequestHandle;
import com.thinkcore.http.core.RequestParams;
import com.thinkcore.http.core.ResponseHandlerInterface;
import com.thinkcore.http.core.RetryHandler;
...
public class TAsyncHttpClient {
	...
	public TAsyncHttpClient(SchemeRegistry schemeRegistry) {
		...
		ConnManagerParams.setMaxConnectionsPerRoute(httpParams,
				new ConnPerRouteBean(mMaxConnections));
		...
		mRequestMap = new WeakHashMap<Context, List<RequestHandle>>();
					...
					if (request.containsHeader(header)) {
						...
						TLog.i(TAG,
								String.format(
										...
										header, mClientHeaderMap.get(header),
										overwritten.getName(),
		...
		mHttpClient.setHttpRequestRetryHandler(new RetryHandler(
	...
	public void setEnableRedirects(final boolean enableRedirects,
		...
		mHttpClient.setRedirectHandler(new MyRedirectHandler(enableRedirects));
	}
	 ...
	 * @see com.treecore.http.core.loopj.android.http.MyRedirectHandler
	 */
	public void setRedirectHandler(final RedirectHandler customRedirectHandler) {
		mHttpClient.setRedirectHandler(customRedirectHandler);
	}
	...
	public void removeHeader(String header) {
		mClientHeaderMap.remove(header);
	}
	...
	public void setAuthenticationPreemptive(boolean isPreemtive) {
		if (isPreemtive) {
			mHttpClient.addRequestInterceptor(
	...
	public void cancelRequests(final Context context,
		...
		Runnable r = new Runnable() {
			@Override
			public void run() {
				List<RequestHandle> requestList = mRequestMap.get(context);
				if (requestList != null) {
	...
	public void cancelAllRequests(boolean mayInterruptIfRunning) {
		for (List<RequestHandle> requestList : mRequestMap.values()) {
			if (requestList != null) {
				for (RequestHandle requestHandle : requestList) {
					requestHandle.cancel(mayInterruptIfRunning);
				}
	...
	public RequestHandle head(Context context, String url,
		...
		return sendRequest(mHttpClient, mHttpContext, new HttpHead(
	...
	public RequestHandle head(Context context, String url, Header[] headers,
		...
		return sendRequest(mHttpClient, mHttpContext, request, null,
	...
	public RequestHandle get(Context context, String url, RequestParams params,
			ResponseHandlerInterface responseHandler) {
		return sendRequest(mHttpClient, mHttpContext, new HttpGet(
				getUrlWithQueryString(isUrlEncodingEnabled, url, params)),
				null, responseHandler, context);
	}
	...
	public RequestHandle post(Context context, String url, Header[] headers,
		...
		if (headers != null)
			request.setHeaders(headers);
		return sendRequest(mHttpClient, mHttpContext, request, contentType,
				responseHandler, context);
	}
	...
	public RequestHandle delete(Context context, String url,
		...
		return sendRequest(mHttpClient, mHttpContext, delete, null,
				responseHandler, context);
	}
	...
	public RequestHandle delete(Context context, String url, Header[] headers,
		...
		return sendRequest(mHttpClient, mHttpContext, httpDelete, null,
				responseHandler, context);
	}
