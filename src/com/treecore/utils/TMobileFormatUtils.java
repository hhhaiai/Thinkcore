...
public class TMobileFormatUtils {
	...
	public void setMobile(String mobile) {
		...
		/** */
		/** 第一步判断中国移动 */
		if (mobile.matches(TMobileFormatUtils.regMobileStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(0);
			this.setLawful(true);
			if (mobile.matches(TMobileFormatUtils.regMobile3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第二步判断中国联通 */
		else if (mobile.matches(TMobileFormatUtils.regUnicomStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(1);
			this.setLawful(true);
			if (mobile.matches(TMobileFormatUtils.regUnicom3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第三步判断中国电脑 */
		else if (mobile.matches(TMobileFormatUtils.regTelecomStr)) {
			this.mobile = mobile;
			this.setFacilitatorType(2);
			this.setLawful(true);
			if (mobile.matches(TMobileFormatUtils.regTelocom3GStr)) {
				this.setIs3G(true);
			}
		}
		/** */
		/** 第四步判断座机 */
		if (mobile.matches(TMobileFormatUtils.regPhoneString)) {
			this.mobile = mobile;
			this.setFacilitatorType(0);
			this.setLawful(true);
			if (mobile.matches(TMobileFormatUtils.regMobile3GStr)) {
