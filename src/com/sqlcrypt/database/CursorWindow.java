 ...
 * When sent to a remote process (by writing it to a {@link Parcel}), the remote
 * process receives a read-only view of the cursor window. Typically the cursor
 * window will be allocated by the producer, filled with data, and then sent to
 * the consumer for reading.
 * </p>
 */
public class CursorWindow extends SQLiteClosable implements Parcelable {
	...
	private static native boolean nativeSetNumColumns(int windowPtr,
			int columnNum);
	...
	private static native byte[] nativeGetBlob(int windowPtr, int row,
			int column);

	private static native String nativeGetString(int windowPtr, int row,
			int column);

	private static native long nativeGetLong(int windowPtr, int row, int column);

	private static native double nativeGetDouble(int windowPtr, int row,
			int column);

	private static native void nativeCopyStringToBuffer(int windowPtr, int row,
			int column, CharArrayBuffer buffer);

	private static native boolean nativePutBlob(int windowPtr, byte[] value,
			int row, int column);

	private static native boolean nativePutString(int windowPtr, String value,
			int row, int column);

	private static native boolean nativePutLong(int windowPtr, long value,
			int row, int column);

	private static native boolean nativePutDouble(int windowPtr, double value,
			int row, int column);

	private static native boolean nativePutNull(int windowPtr, int row,
			int column);
	 ...
	 * {@link #setNumColumns(int)} to set the number of columns before adding
	...
	public CursorWindow(String name) {
		...
		if (mWindowPtr == 0) {
			throw new CursorWindowAllocationException(
					"Cursor window allocation of " + (sCursorWindowSize / 1024)
							+ " kb failed. " + printStats());
		}
	 ...
	 * 
	 * @param localWindow
	 *            True if this window will be used in this process only, false
	 *            if it might be sent to another processes. This argument is
	 *            ignored.
	 * 
	 * @deprecated There is no longer a distinction between local and remote
	 *             cursor windows. Use the {@link #CursorWindow(String)}
	 *             constructor instead.
	 */
	@Deprecated
	...
	/**
	 * Closes the cursor window and frees its underlying resources when all
	 * other remaining references have been released.
	 */
	public void close() {
		releaseReference();
	}
	 ...
	 * for new data.
	 * <p>
	 * The start position ({@link #getStartPosition()}), number of rows (
	 * {@link #getNumRows()}), and number of columns in the cursor are all reset
	 * to zero.
	 * </p>
	 */
	public void clear() {
	 ...
	 * <p>
	 * The start position is the zero-based index of the first row that this
	 * window contains relative to the entire result set of the {@link Cursor}.
	 * </p>
	 * 
	 ...
	 * <p>
	 * This method must be called before any rows are added to the window,
	 * otherwise it will fail to set the number of columns if it differs from
	...
	/**
	 * Returns true if the field at the specified row and column index has type
	 * {@link Cursor#FIELD_TYPE_BLOB} or {@link Cursor#FIELD_TYPE_NULL}.
	 * 
	 * @param row
	 *            The zero-based row index.
	 * @param column
	...
	/**
	 * Returns true if the field at the specified row and column index has type
	 * {@link Cursor#FIELD_TYPE_STRING} or {@link Cursor#FIELD_TYPE_NULL}.
	 * 
	 * @param row
	 *            The zero-based row index.
	 * @param column
	 *            The zero-based column index.
	 * @return True if the field has type {@link Cursor#FIELD_TYPE_STRING} or
	 *         {@link Cursor#FIELD_TYPE_NULL}.
	 * @deprecated Use {@link #getType(int, int)} instead.
	 */
	@Deprecated
	public boolean isString(int row, int column) {
		int type = getType(row, column);
		return type == Cursor.FIELD_TYPE_STRING
	 ...
	 * <ul>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_NULL}, then the
	 * result is <code>null</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_BLOB}, then the
	 * result is the blob value.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_STRING}, then the
	 * result is the array of bytes that make up the internal representation of
	 * the string value.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_INTEGER} or
	 * {@link Cursor#FIELD_TYPE_FLOAT}, then a {@link SQLiteException} is
	 * thrown.</li>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * result is <code>null</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_STRING}, then the
	 * result is the string value.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_INTEGER}, then the
	 * result is a string representation of the integer in decimal, obtained by
	 * formatting the value with the <code>printf</code> family of functions
	 * using format specifier <code>%lld</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_FLOAT}, then the
	 * result is a string representation of the floating-point value in decimal,
	 * obtained by formatting the value with the <code>printf</code> family of
	 * functions using format specifier <code>%g</code>.</li>
	 ...
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_NULL}, then the
	 * buffer is set to an empty string.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_STRING}, then the
	 * buffer is set to the contents of the string.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_INTEGER}, then the
	 * buffer is set to a string representation of the integer in decimal,
	 * obtained by formatting the value with the <code>printf</code> family of
	 * functions using format specifier <code>%lld</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_FLOAT}, then the
	 * buffer is set to a string representation of the floating-point value in
	 * decimal, obtained by formatting the value with the <code>printf</code>
	 * family of functions using format specifier <code>%g</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_BLOB}, then a
	 * {@link SQLiteException} is thrown.</li>
	 * </ul>
	 * </p>
	 ...
	 *            The zero-based column index.
	 * @param buffer
	 *            The {@link CharArrayBuffer} to hold the string. It is
	 *            automatically resized if the requested string is larger than
	...
	public void copyStringToBuffer(int row, int column, CharArrayBuffer buffer) {
		...
		acquireReference();
		try {
			nativeCopyStringToBuffer(mWindowPtr, row - mStartPos, column,
					buffer);
		} finally {
			releaseReference();
		}
	 ...
	 * <ul>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_NULL}, then the
	 * result is <code>0L</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_STRING}, then the
	 * result is the value obtained by parsing the string value with
	 * <code>strtoll</code>.
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_INTEGER}, then the
	 * result is the <code>long</code> value.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_FLOAT}, then the
	 * result is the floating-point value converted to a <code>long</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_BLOB}, then a
	 * {@link SQLiteException} is thrown.</li>
	 * </ul>
	 * </p>
	 ...
	 * <ul>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_NULL}, then the
	 * result is <code>0.0</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_STRING}, then the
	 * result is the value obtained by parsing the string value with
	 * <code>strtod</code>.
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_INTEGER}, then the
	 * result is the integer value converted to a <code>double</code>.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_FLOAT}, then the
	 * result is the <code>double</code> value.</li>
	 * <li>If the field is of type {@link Cursor#FIELD_TYPE_BLOB}, then a
	 * {@link SQLiteException} is thrown.</li>
	 * </ul>
	 * </p>
	 ...
	 * <code>float</code>.
	 * <p>
	 * The result is determined by invoking {@link #getDouble} and converting
	 * the result to <code>float</code>.
	 * </p>
	 * 
	 * @param row
	 *            The zero-based row index.
	...
	private void recordNewWindow(int pid, int window) {
		synchronized (sWindowToPidMap) {
			...
			TLog.i(STATS_TAG, "Created a new Cursor. " + printStats());
		}
	}
	...
	private String printStats() {
		...
		String s = (buff.length() > 980) ? buff.substring(0, 980) : buff
				.toString();
