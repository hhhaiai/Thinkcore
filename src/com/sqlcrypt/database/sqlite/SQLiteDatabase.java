...
import com.sqlcrypt.database.sqlite.SQLiteDebug.DbStats;
...
public class SQLiteDatabase extends SQLiteClosable {
     ...
     * @param passwd if the database not need the password, set it null 
     ...
     */
    public static SQLiteDatabase openDatabase(String path, String passwd, CursorFactory factory, int flags) {
        return openDatabase(path, passwd, factory, flags, new DefaultDatabaseErrorHandler());
    }
    ...
    public static SQLiteDatabase openDatabase(String path, String passwd, CursorFactory factory, int flags,
        ...
        SQLiteDatabase sqliteDatabase = openDatabase(path, passwd, factory, flags, errorHandler,
    ...
    private static SQLiteDatabase openDatabase(String path, String passwd, CursorFactory factory, int flags,
            DatabaseErrorHandler errorHandler, short connectionNum) {
        SQLiteDatabase db = new SQLiteDatabase(path, passwd, factory, flags, errorHandler, connectionNum);
        try {
            if (Log.isLoggable(TAG, Log.DEBUG)) {
            	TLog.i(TAG, "opening the db : " + path);
            }
            // Open the database.
            db.dbopen(path, flags);
            if(passwd != null) {
            	db.native_key(passwd);
            }
            ...
            return SQLiteDatabase.openDatabase(path, passwd, factory, flags, errorHandler);
        } catch (SQLiteException e) {
        	TLog.e(TAG, "Failed to open the database. closing it.", e);
            db.close();
            throw e;
        }
    }
    ...
    public static SQLiteDatabase openOrCreateDatabase(String path, CursorFactory factory) {
        return openDatabase(path, null, factory, CREATE_IF_NECESSARY);
    }
    ...
    public static SQLiteDatabase openOrCreateDatabase(String path, CursorFactory factory,
            DatabaseErrorHandler errorHandler) {
        return openDatabase(path, null, factory, CREATE_IF_NECESSARY, errorHandler);
    }
    ...
    private void setJournalMode(final String dbPath, final String mode) {
        ...
        if (!s.equalsIgnoreCase(mode)) {
        	TLog.e(TAG, "setting journal_mode to " + mode + " failed for db: " + dbPath +
    ...
    public static SQLiteDatabase create(CursorFactory factory) {
        ...
        return openDatabase(MEMORY_DB_PATH, null, factory, CREATE_IF_NECESSARY);
    }
    ...
    public void close() {
        ...
        if (Log.isLoggable(TAG, Log.DEBUG)) {
        	TLog.i(TAG, "closing db: " + mPath + " (connection # " + mConnectionNum);
        }
        lock();
        try {
            ...
            if (mConnectionPool != null) {
                if (Log.isLoggable(TAG, Log.DEBUG)) {
                    ...
                    TLog.i(TAG, mConnectionPool.toString());
                }
        ...
        try {
            ...
            if (!found) {
                ...
                if (mClosedStatementIds.contains(stmtId)) {
                	TLog.w(TAG, "this shouldn't happen. finalizing the statement now: ");
                    closePendingStatements();
    ...
    public long insert(String table, String nullColumnHack, ContentValues values) {
        try {
        	...
        	TLog.e(TAG, "Error inserting " + values, e);
            return -1;
        }
    }
    ...
    public long replace(String table, String nullColumnHack, ContentValues initialValues) {
        try {
        	...
        	TLog.e(TAG, "Error inserting " + initialValues, e);
            return -1;
        }
    }
    ...
    protected void finalize() throws Throwable {
        try {
            if (isOpen()) {
            	TLog.e(TAG, "close() was never explicitly called on database '" +
                        mPath + "' ", mStackTrace);
                closeClosable();
     ...
     * connection handles.
     */
    private SQLiteDatabase(String path, String passwd, CursorFactory factory, int flags,
            DatabaseErrorHandler errorHandler, short connectionNum) {
        if (path == null) {
    ...
    private void logTimeStat(String sql, long beginMillis, String prefix) {
        ...
        String blockingPackage = "unknown";//AppGlobals.getInitialPackage();
    ...
    /* package */ synchronized void addToCompiledQueries(
        ...
        if (SQLiteDebug.DEBUG_SQL_CACHE) {
            ...
            if (printWarning) {
                 ...
                 */
            	TLog.w(TAG, "Reached MAX size for compiled-sql statement cache for database " +
                        getPath() + ". Use setMaxSqlCacheSize() to increase cachesize. ");
                mCacheFullWarning = true;
                TLog.d(TAG, "Here are the SQL statements in Cache of database: " + mPath);
                for (String s : mCompiledQueries.snapshot().keySet()) {
                	TLog.d(TAG, "Sql statement in Cache: " + s);
                }
            }
        }
    ...
    public boolean enableWriteAheadLogging() {
        ...
        try {
            ...
            if (mPath.equalsIgnoreCase(MEMORY_DB_PATH)) {
            	TLog.i(TAG, "can't enable WAL for memory databases.");
                return false;
            }
    ...
    /* package */ SQLiteDatabase createPoolConnection(short connectionNum) {
        SQLiteDatabase db = openDatabase(mPath, mPasswd, mFactory, mFlags, mErrorHandler, connectionNum);
    ...
    private void releaseDbConnection(SQLiteDatabase db) {
        ...
        if (Log.isLoggable(TAG, Log.DEBUG)) {
            ...
            assert mConnectionPool != null;
            TLog.d(TAG, "releaseDbConnection threadid = " + Thread.currentThread().getId() +
        ...
        for (int i = 0; i < attachedDbs.size(); i++) {
            ...
            try {
                ...
                if (!rslt.equalsIgnoreCase("ok")) {
                	...
                	TLog.e(TAG, "PRAGMA integrity_check on " + p.second + " returned: " + rslt);
                    return false;
                }
            } finally {
     ...
     * @author leon
     * */
    public void resetPassword(String passwd) {
    	if(passwd != null) {
    		native_rekey(passwd);
    	}
    }
    ...
    /**
     * Set up the password in database
     * @author leon
     * */
    private native void native_key(String passwd);
    /**
     * reset the password
     * @author leon
     * */
    private native void native_rekey(String passwd);
