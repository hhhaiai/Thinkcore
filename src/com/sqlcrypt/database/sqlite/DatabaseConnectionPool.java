...
/**
 * A connection pool to be used by readers. Note that each connection can be
 * used by only one reader at a time.
 */
/* package */class DatabaseConnectionPool {
	...
	/**
	 * The connection pool objects are stored in this member. TODO: revisit this
	 * data struct as the number of pooled connections increase beyond
	 * single-digit values.
	 */
	private final ArrayList<PoolObj> mPool = new ArrayList<PoolObj>(
			mMaxPoolSize);
	...
	/* package */DatabaseConnectionPool(SQLiteDatabase db) {
		...
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			TLog.d(TAG, "Max Pool Size: " + mMaxPoolSize);
		}
	}
	...
	/* package */synchronized void close() {
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			TLog.d(TAG,
					"Closing the connection pool on " + mParentDbObj.getPath()
							+ toString());
		}
	 ...
	 * 
	 * @param sql
	 *            if not null, try to find a connection inthe pool which already
	 *            has cached the compiled statement for this sql.
	...
	/* package */synchronized SQLiteDatabase get(String sql) {
		...
		if (getFreePoolSize() == 0) {
			...
			if (mMaxPoolSize == poolSize) {
				...
				// preferably a connection caching the pre-compiled statement of
				// the given SQL
				if (mMaxPoolSize == 1) {
					poolObj = mPool.get(0);
				} else {
					...
					if (poolObj == null) {
						// there are no database connections with the given SQL
						// pre-compiled.
				...
				// create a new connection and add it to the pool, since we
				// haven't reached
			...
			// preferably a connection caching the pre-compiled statement of the
			// given SQL
			for (int i = 0; i < poolSize; i++) {
				if (mPool.get(i).isFree()
						&& mPool.get(i).mDb.isInStatementCache(sql)) {
					poolObj = mPool.get(i);
					break;
				}
			}
			if (poolObj == null) {
				// didn't find a free database connection with the given SQL
				// already
				// pre-compiled. return a free connection (this means, the same
				// SQL could be
				// pre-compiled on more than one database connection. potential
				// wasted memory.)
				for (int i = 0; i < poolSize; i++) {
					if (mPool.get(i).isFree()) {
						poolObj = mPool.get(i);
		...
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			TLog.d(TAG,
					"END get-connection: " + toString() + poolObj.toString());
		}
		return db;
		// TODO if a thread acquires a connection and dies without releasing the
		// connection, then
	...
	/* package */synchronized void release(SQLiteDatabase db) {
		...
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			TLog.d(TAG,
					"BEGIN release-conn: " + toString() + poolObj.toString());
		}
		...
		poolObj.release();
		if (Log.isLoggable(TAG, Log.DEBUG)) {
			TLog.d(TAG, "END release-conn: " + toString() + poolObj.toString());
		}
	}

	/**
	 * Returns a list of all database connections in the pool (both free and
	 * busy connections). This method is used when "adb bugreport" is done.
	 */
	...
	/**
	 * package level access for testing purposes only. otherwise, private should
	 * be sufficient.
	 */
	...
	/**
	 * represents objects in the connection pool. package-level access for
	...
	/* package */static class PoolObj {
		...
		/**
		 * contains the threadIds of the threads holding this connection. used
