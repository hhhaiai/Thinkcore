...
/**
 * This class encapsulates compilation of sql statement and release of the
 * compiled statement obj. Once a sql statement is compiled, it is cached in
 * {@link SQLiteDatabase} and it is released in one of the 2 following ways 1.
 * when {@link SQLiteDatabase} object is closed. 2. if this is not cached in
 * {@link SQLiteDatabase}, {@link android.database.Cursor#close()} releaases
 * this obj.
 */
/* package */class SQLiteCompiledSql {
	...
	/**
	 * Native linkage, do not modify. When non-0 this holds a reference to a
	 * valid sqlite3_statement object. It is only updated by the native code,
	 * but may be checked in this class when the database lock is held to
	 * determine if there is a valid native-side program or not.
	 */
	...
	/* package */SQLiteCompiledSql(SQLiteDatabase db, String sql) {
		...
		/*
		 * if (StrictMode.vmSqliteObjectLeaksEnabled()) { mStackTrace = new
		 * DatabaseObjectNotClosedException().fillInStackTrace(); } else {
		 * mStackTrace = null; }
		 */
	...
	protected void finalize() throws Throwable {
		try {
			...
			// all sub-objects attached to the database could end up getting
			// GC'ed too.
			...
			if (mInUse && mStackTrace != null) {
				...
				TLog.w(TAG,
						"Releasing statement in a finalizer. Please ensure "
								...
								+ mSqlStmt.substring(0, (len > 1000) ? 1000
										: len) + mStackTrace);
			}
			releaseSqlStatement();
		} finally {
