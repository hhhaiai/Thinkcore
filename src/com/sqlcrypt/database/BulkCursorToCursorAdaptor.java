...
public final class BulkCursorToCursorAdaptor extends AbstractWindowedCursor {
	...
	/**
	 * Gets a SelfDataChangeOberserver that can be sent to a remote process to
	 * receive change notifications over IPC.
	 * 
	...
	public void deactivate() {
		// This will call onInvalidated(), so make sure to do it before calling
		// release,
	...
	public void close() {
		...
		if (mBulkCursor != null) {
			try {
				...
				TLog.w(TAG, "Remote process exception when closing");
			} finally {
				mBulkCursor = null;
			}
		}
	...
	public boolean requery() {
		...
		try {
			...
			if (mCount != -1) {
				...
				// super.requery() will call onChanged. Do it here instead of
				// relying on the
				// observer from the far side so that observers can see a
				// correct value for mCount
	...
	public Bundle getExtras() {
		...
		try {
			...
			// This should never happen because the system kills processes that
			// are using remote
	...
	public Bundle respond(Bundle extras) {
		...
		try {
			...
			// the system kills processes that are using remote cursors when the
			// provider process
			// is killed, but this can still happen if this is being called from
			// the system process,
